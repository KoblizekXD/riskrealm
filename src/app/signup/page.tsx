...
export default function SignUp(props: { searchParams: { callbackUrl: string | undefined, error: string | undefined } }) {
  ...
  return (
    <main className={'min-h-screen md:h-screen lg:bg-black from-[#1e1e2e] to-[#181825] bg-gradient-to-br flex justify-center p-16 items-center'}>
      {error && <GenericErrorPopup onClose={() => setError(undefined)} closeable message={translateError(error)} />}
      <div className={'w-full h-full z-10 flex flex-col gap-x-2 lg:flex-row p-2'}>
          ...
          <form action={(fd: FormData) => {
            ...
            <label className={'mt-4 font-semibold'}>
              ...
              <input
                ...
                type={'password'}
                name={'password'}
                className={'block border-[#5d5e6e] border bg-[#11111b] rounded w-full p-2 mt-1 outline-none'}
              />
            </label>
        ...
        <div className={'flex select-none lg:flex-col items-center gap-x-2 gap-y-2 py-4'}>
