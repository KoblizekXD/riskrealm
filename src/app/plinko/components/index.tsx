...
export function Plinko() {
  // #region States
  const incrementCurrentBalance = useAuthStore((state: { incrementBalance: any }) => state.incrementBalance)
  const engine = Engine.create()
  const [lines, setLines] = useState<LinesType>(16)
  const inGameBallsCount = useGameStore((state: { gamesRunning: any }) => state.gamesRunning)
  const incrementInGameBallsCount = useGameStore(
    (    state: { incrementGamesRunning: any }) => state.incrementGamesRunning
  )
  const decrementInGameBallsCount = useGameStore(
    (    state: { decrementGamesRunning: any }) => state.decrementGamesRunning
  )
