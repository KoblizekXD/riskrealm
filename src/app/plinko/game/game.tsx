...
export function Plinko() {
  ...
  const [playerBalance, setPlayerBalance] = useState<number>(100000);
  ...
  function BetActions({
    ...
    async function handleRunBet() {
      ...
      //setPlayerBalance(await decrementBalance(betValue, playerBalance));
    }
    ...
    return (
      <div className="relative h-1/2 w-full flex-1 py-8 px-4 bg-cyan-500">
        ...
        <span className="left-4 top-0 mx-auto text-xs font-bold text-black md:text-base">
         Balance: {playerBalance.toFixed(0)}
        </span>
  ...
  const addBall = useCallback(
    async (ballValue: number) => {
      ...
      //console.log(`old balance: ${playerBalance}`);
  ...
  const onCollideWithMultiplier = useCallback(
    async (ball: Body, multiplier: Body) => {
      ...
      const newWin = await calcWin(multiplierValue, +ballValue);
      console.log(`New win: ${newWin}`);
      console.log(`old balance: ${playerBalance}`);
      setPlayerBalance(playerBalance + newWin);
      await console.log(`New balance: ${playerBalance}`);
